...
if (isset($_POST['yearlyContributionSummary'])) {
	$year = $_POST['year'];
	$options = array();
	if(isset($_POST['AMF'])){
	...
	<body>
		<div class="container">
			...
			<div class="row mt-4">
				<div class="col h5">CONTRIBUTION SUMMARY (<?php echo $year ?>)</div>
			</div>
				...
				<table class="table table-responsive-md table-bordered bg-white rounded shadow-sm  table-striped text-center" id="summaryTbl">
					...
					<tbody>
						...
						if(count($options) > 0){
							...
							$query .= " FROM contribution_summary JOIN users ON contribution_summary.empNo = users.empNo WHERE YEAR(contribution_summary.date) = $year ";
							foreach($options as $option){
								if($option == $options[0]){
									$query .= " AND (".$option." != 0";
								}
							...
							if(mysqli_num_rows($result) > 0){
								...
								echo '<tr><td colspan="7">No contributions has been made in this year ('.$year.').</td></tr>';
							}
							?>
						</tbody>
					</table>
