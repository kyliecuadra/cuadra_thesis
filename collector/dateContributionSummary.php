...
if (isset($_POST['dateContributionSummary'])) {
	$fromDate = $_POST['from'];
	...
	<body>
		<div class="container">
			...
			<div class="row mt-4">
				<div class="col h5">CONTRIBUTION SUMMARY (<?php echo $fdate .' - '. $tdate; ?>)</div>
			</div>
			<div class="row pl-2 pr-2">
				<table class="table table-responsive-md table-bordered bg-white rounded shadow-sm  table-striped text-center" id="summaryTbl">
					...
					<tbody>
						<?php 
						if(count($options) > 0){
							$query = "SELECT contribution_summary.empNo, contribution_summary.name, contribution_summary.date, users.staffLvl,";
							foreach($options as $option){
								$query .= $option.", ";
							}
							foreach($options as $option){
								$query .= "SUM(contribution_summary.".$option.") AS ". $option."_sum, ";
							}
							$query = substr($query, 0, -2);
							$query .= " FROM contribution_summary JOIN users ON contribution_summary.empNo = users.empNo WHERE contribution_summary.date BETWEEN '$fromDate' AND '$toDate' ";
							foreach($options as $option){
								if($option == $options[0]){
									$query .= " AND (".$option." != 0";
								}
							...
							$query .= ") AND users.staffLvl = '".$_SESSION['staffLvl']."' GROUP BY name";
							...
							if(mysqli_num_rows($result) > 0){
								...
								echo '<tr><td colspan="7">No contributions has been made within this date ('.$fdate.' - '.$tdate.').</td></tr>';
							}
							?>
						</tbody>
					</table>
