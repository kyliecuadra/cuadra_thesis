...
if (isset($_POST['dateExpenses'])) {
	...
	<body>
		<div class="container">
			...
			<div class="row mt-4">
				<div class="col h5">EXPENSES (<?php echo $fdate .' - '. $tdate; ?>)</div>
			</div>
			<div class="row pl-2 pr-2">
				<table class="table table-responsive-md table-bordered bg-white rounded shadow-sm  table-striped text-center" id="summaryTbl">
					...
					<tbody>
						<?php 
						$displayQuery = mysqli_query($con, "SELECT CONCAT(users.fname,' ', users.mname,' ', users.lname,' ', users.suffix) as Name, users.status, users.staffLvl, expenses.empNo, expenses.name, expenses.reason, expenses.amount, expenses.date FROM users JOIN expenses ON expenses.empNo = users.empNo WHERE expenses.date BETWEEN '$fromDate' AND '$toDate' AND users.staffLvl = '".$_SESSION['staffLvl']."' ORDER BY users.fname ASC, expenses.date DESC");
						if(mysqli_num_rows($displayQuery) > 0){
							...
							echo '<tr><td colspan="5">No contributions has been made within this date ('.$fdate.' - '.$tdate.').</td></tr>';
						}
						?>
					</tbody>
				</table>
