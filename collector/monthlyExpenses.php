...
if (isset($_POST['monthlyExpenses'])) {
	$month = $_POST['month'];
	...
	<body>
		<div class="container">
			...
			<div class="row mt-4">
				<div class="col h5">EXPENSES (<?php echo date('F, Y' , strtotime($month)); ?>)</div>
			</div>
			<div class="row pl-2 pr-2">
				<table class="table table-responsive-md table-bordered bg-white rounded shadow-sm  table-striped text-center" id="summaryTbl">
					...
					<tbody>
						<?php 
						$displayQuery = mysqli_query($con, "SELECT expenses.*, users.staffLvl FROM expenses JOIN users ON expenses.empNo = users.empNo  WHERE users.staffLvl = '".$_SESSION['staffLvl']."' AND DATE_FORMAT(expenses.date, '%m-%Y') = '$fmonth' ORDER BY expenses.date DESC");
						if(mysqli_num_rows($displayQuery) > 0){
							...
							echo '<tr><td colspan="5">No contributions has been made in this month ('.date('F, Y' , strtotime($month)).').</td></tr>';
						}
						?>
					</tbody>
				</table>
