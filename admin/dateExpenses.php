		...
		<div class="container">
			...
			<div class="row pl-2 pr-2">
				<table class="table table-responsive-md table-bordered bg-white rounded shadow-sm  table-striped text-center" id="summaryTbl">
					...
					<tbody>
						<?php 
						$displayQuery = mysqli_query($con, "SELECT * FROM expenses WHERE date BETWEEN '$fromDate' AND '$toDate' ORDER BY date DESC");
						if(mysqli_num_rows($displayQuery) > 0){
							...
							echo '<tr><td colspan="5">No contributions has been made in this date ('.$fdate.' - '.$tdate.').</td></tr>';
						}
						?>
					</tbody>
				</table>
