...
if(isset($_POST['year'])){
	...
	<tbody>';
	$displayQuery = mysqli_query($con, "SELECT CONCAT(users.fname,' ', users.mname,' ', users.lname,' ', users.suffix) as Name, users.status, contribution_records.empNo, contribution_records.name, contribution_records.contributionType, contribution_records.amount, contribution_records.month, contribution_records.year, contribution_records.date, contribution_records.collected_by FROM users JOIN contribution_records ON contribution_records.empNo = users.empNo WHERE contribution_records.year = $year ORDER BY users.fname ASC, contribution_records.date DESC");
	$i=0;
	$prevName='';
